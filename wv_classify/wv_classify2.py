...
def process_file(
    ...
    if d_t > 0:
        ...
        # === Edge Detection
        #img_sub = Rrs[:, :, 5]
        #img_sub = img_sub[numpy.logical_not(numpy.isnan(img_sub))]
...
#        import pdb; pdb.set_trace()
        ...
        if numpy.isnan(avg_water_sum) == True: avg_water_sum = [0]
#        if cl_cov > 0:
...
#        else:
        ...
        Bathy = numpy.zeros((szA[0], szA[1]), dtype=numpy.float)  # Preallocate for Bathymetry
#        Rrs_deglint = float(zeros(5, 1))  # Preallocate for deglinted Rrs
        # Preallocate water-column corrected Rrs
        Rrs_0 = numpy.zeros((5, 1), dtype=numpy.float)
